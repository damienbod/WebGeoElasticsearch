...
public static class HostingElasticExtensions
{
        ...
        where TResource : IResourceWithEnvironment
    {
        ...
        if (builder.ApplicationBuilder.ExecutionContext.IsRunMode)
        {
            ...
            builder
                ...
                .WithEnvironment("xpack.security.http.ssl.certificate", $"{DEV_CERT_DIR}/dev-elastic-cert.pem")
                .WithEnvironment("xpack.security.http.ssl.key", $"{DEV_CERT_DIR}/dev-elastic-cert.key");
        }
    ...
    private static (string, string) ExportElasticDevCertificate(IDistributedApplicationBuilder builder)
    {
        ...
        var certExportPath = Path.Combine(tempDir, "dev-elastic-cert.pem");
        var certKeyExportPath = Path.Combine(tempDir, "dev-elastic-cert.key");
